# NG Tertiary Referral Registry Service Capability - Nigeria Core - FHIR Implementation Guide v0.0.0

* [**Table of Contents**](toc.md)
* [**Artifacts Summary**](artifacts.md)
* **NG Tertiary Referral Registry Service Capability**

## CapabilityStatement: NG Tertiary Referral Registry Service Capability 

| | |
| :--- | :--- |
| *Official URL*:https://fhir-ig.digitalhealth.gov.ng/CapabilityStatement/NgTertiaryRegistryCapability | *Version*:0.0.0 |
| Active as of 2026-08-09 | *Computable Name*:NgTertiaryRegistryCapability |
| *Other Identifiers:*OID:2.16.840.1.113884.3.9944.1.13.12 | |
| **Usage:**Clinical Venue:  | |

 
Minimum computable FHIR requirements for the Nigeria Core Registry Service role in tertiary referral facilities. 

 
Defines the computable Nigeria Core requirements for the Nigeria Core Registry Service role in tertiary referral facilities. Supports patient facility workforce service-location and referral-network discovery. 
This CapabilityStatement supports implementation, procurement, vendor onboarding, SANDBOX FHIR configuration, interoperability testing, and automated conformance assessment. A deployed server SHALL expose only capabilities actually implemented by the deployment in its runtime /metadata response. 
 ...
 [Raw OpenAPI-Swagger Definition file](NgTertiaryRegistryCapability.openapi.json) | [Download](NgTertiaryRegistryCapability.openapi.json) 

## Nigeria Core Tertiary Referral Registry Service Capability
...
| `Provenance` | Base FHIR resource | `target (reference)``recorded (date)``agent (reference)` | `$validate`— HTTP 200 with`OperationOutcome`validation result |
...
```json
{
  ...
  "id" : "NgTertiaryRegistryCapability",
  "url" : "https://fhir-ig.digitalhealth.gov.ng/CapabilityStatement/NgTertiaryRegistryCapability",
  "version" : "0.0.0",
  "name" : "NgTertiaryRegistryCapability",
  "title" : "Nigeria Core Tertiary Referral Registry Service Capability",
  "status" : "active",
  "experimental" : false,
  "date" : "2026-08-09",
  "publisher" : "NDHI",
  ...
  "rest" : [{
    ...
    },
    {
      ...
      "documentation" : "Nigeria Core HealthcareService read/search capability. Reference Policy: literal and logical references are supported. Successful read/vread returns HTTP 200 with HealthcareService; successful search returns HTTP 200 with Bundle.type = searchset. Failure SHALL return HTTP 4xx/5xx with OperationOutcome.",
      "interaction" : [{
        "code" : "read",
    ...
    },
    {
      ...
      "documentation" : "Nigeria Core Provenance read/search capability. Reference Policy: literal and logical references are supported. Successful read/vread returns HTTP 200 with Provenance; successful search returns HTTP 200 with Bundle.type = searchset. Failure SHALL return HTTP 4xx/5xx with OperationOutcome.",
      "interaction" : [{
        "code" : "read",
