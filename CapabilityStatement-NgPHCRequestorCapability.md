# NG PHC Requestor Capability - Nigeria Core - FHIR Implementation Guide v0.0.0

* [**Table of Contents**](toc.md)
* [**Artifacts Summary**](artifacts.md)
* **NG PHC Requestor Capability**

## CapabilityStatement: NG PHC Requestor Capability 

| | |
| :--- | :--- |
| *Official URL*:https://fhir-ig.digitalhealth.gov.ng/CapabilityStatement/NgPHCRequestorCapability | *Version*:0.0.0 |
| Active as of 2026-08-09 | *Computable Name*:NgPHCRequestorCapability |
| *Other Identifiers:*OID:2.16.840.1.113884.3.9944.1.13.12 | |
| **Usage:**Clinical Venue:  | |

 
Minimum computable FHIR requirements for the Nigeria Core Requestor role in primary health care facilities. 

 
Defines the computable Nigeria Core requirements for the Nigeria Core Requestor role in primary health care facilities. Supports routine patient administration and primary clinical service exchange. 
This CapabilityStatement supports implementation, procurement, vendor onboarding, SANDBOX FHIR configuration, interoperability testing, and automated conformance assessment. A deployed server SHALL expose only capabilities actually implemented by the deployment in its runtime /metadata response. 
 ...
 [Raw OpenAPI-Swagger Definition file](NgPHCRequestorCapability.openapi.json) | [Download](NgPHCRequestorCapability.openapi.json) 

## Nigeria Core PHC Requestor Capability
...
| `Observation` | `NgObservation` | `patient (reference)``category (token)``code (token)``date (date)``status (token)` | `$validate`— HTTP 200 with`OperationOutcome`validation result |
...
| `QuestionnaireResponse` | `NgQuestionnaireResponse` | `questionnaire (reference)``patient (reference)``authored (date)``status (token)` | `$validate`— HTTP 200 with`OperationOutcome`validation result |
...
```json
{
  ...
  "id" : "NgPHCRequestorCapability",
  "url" : "https://fhir-ig.digitalhealth.gov.ng/CapabilityStatement/NgPHCRequestorCapability",
  "version" : "0.0.0",
  "name" : "NgPHCRequestorCapability",
  "title" : "Nigeria Core PHC Requestor Capability",
  "status" : "active",
  "experimental" : false,
  "date" : "2026-08-09",
  "publisher" : "NDHI",
