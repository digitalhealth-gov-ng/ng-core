# NG Tertiary Referral Conformance and Sandbox Capability - Nigeria Core - FHIR Implementation Guide v0.0.0

* [**Table of Contents**](toc.md)
* [**Artifacts Summary**](artifacts.md)
* **NG Tertiary Referral Conformance and Sandbox Capability**

## CapabilityStatement: NG Tertiary Referral Conformance and Sandbox Capability 

| | |
| :--- | :--- |
| *Official URL*:https://fhir-ig.digitalhealth.gov.ng/CapabilityStatement/NgTertiarySandboxCapability | *Version*:0.0.0 |
| Active as of 2026-08-09 | *Computable Name*:NgTertiarySandboxCapability |
| *Other Identifiers:*OID:2.16.840.1.113884.3.9944.1.13.15 | |
| **Usage:**Clinical Venue:  | |

 
Minimum computable FHIR requirements for the Nigeria Core Conformance and Sandbox Service role in tertiary referral facilities. 

 
Defines the computable Nigeria Core requirements for the Nigeria Core Conformance and Sandbox Service role in tertiary referral facilities. Supports comprehensive artefact discovery terminology validation operation inspection search testing and IG inspection. 
This CapabilityStatement supports implementation, procurement, vendor onboarding, SANDBOX FHIR configuration, interoperability testing, and automated conformance assessment. A deployed server SHALL expose only capabilities actually implemented by the deployment in its runtime /metadata response. 
 ...
 [Raw OpenAPI-Swagger Definition file](NgTertiarySandboxCapability.openapi.json) | [Download](NgTertiarySandboxCapability.openapi.json) 

## Nigeria Core Tertiary Referral Conformance and Sandbox Capability
...
| `CapabilityStatement` | Base FHIR resource | `url (uri)``name (string)``status (token)` | `$validate`— HTTP 200 with`OperationOutcome`validation result`$implements`— HTTP 200 with`OperationOutcome`containing no issue of severity`error`or`fatal`when requested capabilities are implemented |
...
| `TestScript` | Base FHIR resource | *None beyond common searches* | `$validate`— HTTP 200 with`OperationOutcome`validation result |
| `ImplementationGuide` | Base FHIR resource | *None beyond common searches* | `$validate`— HTTP 200 with`OperationOutcome`validation result |
...
```json
{
  ...
  "id" : "NgTertiarySandboxCapability",
  "url" : "https://fhir-ig.digitalhealth.gov.ng/CapabilityStatement/NgTertiarySandboxCapability",
  "version" : "0.0.0",
  "name" : "NgTertiarySandboxCapability",
  "title" : "Nigeria Core Tertiary Referral Conformance and Sandbox Capability",
  "status" : "active",
  "experimental" : false,
  "date" : "2026-08-09",
  "publisher" : "NDHI",
  ...
  "rest" : [{
    ...
    {
      "type" : "TestScript",
      "documentation" : "Nigeria Core TestScript read/search capability. Reference Policy: literal and logical references are supported. Successful read/vread returns HTTP 200 with TestScript; successful search returns HTTP 200 with Bundle.type = searchset. Failure SHALL return HTTP 4xx/5xx with OperationOutcome.",
      "interaction" : [{
        "code" : "read",
        "documentation" : "Success: HTTP 200 with the requested TestScript. Failure: HTTP 4xx/5xx with OperationOutcome."
      },
      {
        "code" : "vread",
        "documentation" : "Success: HTTP 200 with the requested historical TestScript version. Failure: HTTP 4xx/5xx with OperationOutcome."
      },
      {
        "code" : "search-type",
        "documentation" : "Success: HTTP 200 with Bundle.type = searchset. Failure: HTTP 4xx/5xx with OperationOutcome."
    ...
    },
    {
      ...
      "documentation" : "Nigeria Core ImplementationGuide read/search capability. Reference Policy: literal and logical references are supported. Successful read/vread returns HTTP 200 with ImplementationGuide; successful search returns HTTP 200 with Bundle.type = searchset. Failure SHALL return HTTP 4xx/5xx with OperationOutcome.",
      "interaction" : [{
        "code" : "read",
        "documentation" : "Success: HTTP 200 with the requested ImplementationGuide. Failure: HTTP 4xx/5xx with OperationOutcome."
      },
      {
        "code" : "vread",
        "documentation" : "Success: HTTP 200 with the requested historical ImplementationGuide version. Failure: HTTP 4xx/5xx with OperationOutcome."
      },
      {
        "code" : "search-type",
        "documentation" : "Success: HTTP 200 with Bundle.type = searchset. Failure: HTTP 4xx/5xx with OperationOutcome."
